...
-- a swap after elimination has begun; the previous `L` multipliers must follow the rows
example : Matrix.det (R := ℚ) !![1, 1, 0; 2, 2, 1; 3, 4, 0] = -1 := by
  lu_det

-- two row swaps
example : Matrix.det (R := ℚ) !![0, 1, 0; 0, 0, 1; 1, 0, 0] = 1 := by
  lu_det

-- the certificate is computed over `ℚ`, but the matrix can live in another
-- characteristic-zero field
example : Matrix.det (R := ℝ) !![1 / 2, 2; 3, 4] = -4 := by
  lu_det

example : Matrix.det (R := ℂ) !![1, 2; 3, 4] = -2 := by
  lu_det
